# Binance SMA + 메이저 코인 Portfolio Paper Trader

이 프로젝트에는 두 실행기가 있습니다. `sma-bot`은 BTC/USDT 같은 단일 현물의 SMA 교차를 paper 또는 Binance Spot Testnet에서 실행합니다. `portfolio-paper`는 백테스트에서 선별한 EMA 50/180·역변동성 메이저 코인 전략을 **실제 주문 없이** 실시간 공개 시세로 모의운용합니다.
...
```bash
git clone https://github.com/whtjddlr/binance-sma-bot.git
cd binance-sma-bot
python -m venv .venv
...
## 최적화된 메이저 코인 전략 실시간 모의운용

`portfolio-paper`는 BTC, ETH, BNB, SOL, XRP, ADA, DOGE, LINK의 **확정된 UTC 일봉만** 사용합니다. 전략은 EMA 50/180, 20일 장기선 기울기, 90일 모멘텀, 30일 역변동성 배분을 사용하며 총 투자비중은 25%, 코인당 최대 비중은 10%입니다. Binance API 키와 주문 권한은 사용하지 않습니다.

```bash
cp .env.portfolio.example .env.portfolio
portfolio-paper check
portfolio-paper once
portfolio-paper status
```

최초 `once`는 최신 일봉을 기준점으로 저장할 뿐 과거 신호를 현재 가격으로 뒤늦게 체결하지 않습니다. 다음 확정 일봉부터 하루 한 번 처리하고 월말에 종목을 다시 선정합니다. 계속 감시하려면 실행합니다.

```bash
portfolio-paper run
```

주요 상태는 `data/portfolio-paper-state.json`에 원자적으로 저장됩니다. 같은 일봉은 다시 체결하지 않으며, 중간에 이틀 이상 실행하지 못해 일봉 처리를 놓치면 현재 가격으로 과거 체결을 꾸며내지 않고 중지합니다. 상태 파일은 임의 수정하거나 삭제하지 마세요.

위험 규칙은 10% 사이클 손실 바닥, 수익 발생 후 25% 이익 보호, 15일 재진입 대기, 누적 최대낙폭 20% 영구 중지입니다. 모두 일봉과 로컬 모의 장부를 기준으로 하며 손실 한도를 보장하지 않습니다.

### Docker로 24시간 실행

전용 portfolio 환경파일을 사용하므로 단일 SMA Testnet 키를 컨테이너에 전달하지 않습니다.

```bash
cp .env.portfolio.example .env.portfolio
docker compose build
docker compose run --rm portfolio-paper portfolio-paper check
docker compose run --rm portfolio-paper portfolio-paper once
docker compose up -d
docker compose logs -f portfolio-paper
```

실행 중 상태 확인과 종료는 다음과 같습니다.

```bash
docker compose exec portfolio-paper portfolio-paper status
docker compose down
```
...
`portfolio-paper`는 현재 선별된 후보를 전진 모의운용하는 실행기이고, `research_optimizer.py`는 후보를 다시 탐색하는 별도 도구입니다. 저장소에는 대용량 과거 시세 JSON을 포함하지 않으므로 최적화를 재실행하려면 `data/research`에 개발·검증 기간을 덮는 Binance 일봉 데이터가 필요합니다. 반면 `portfolio-paper`는 실행 시 공개 API에서 필요한 최근 일봉을 직접 받습니다.
...
기존 연구 데이터가 준비된 환경에서 새 일봉이 추가됐을 때만 갱신하고 재탐색하려면 다음 연구 루프를 별도 서버나 작업 스케줄러에서 실행합니다.
...
- 모든 단위 테스트는 로컬과 GitHub Actions에서 실행되도록 구성했습니다.
- 2024~2025 미사용 구간에서 기록한 +8.48%는 과거 시뮬레이션이며 실거래 수익이 아닙니다.
- 해당 결과를 확인한 뒤에는 같은 기간이 더 이상 완전히 새로운 미사용 구간이 아니므로, `portfolio-paper` 상태를 보존하며 앞으로 쌓이는 일봉으로 전진검증해야 합니다.
- `portfolio-paper`에는 실제 주문 코드가 없으며, `sma-bot`의 Testnet 실행기와 상태 파일도 공유하지 않습니다.
